...
class Grupo:
	...
	def __init__(self, arquivo, diretorio=None, membro=None, area=None):
		...
		if area:
			self.atualizarParametro('global-arquivo_qualis_de_periodicos', "%s.csv" % area)
		...
		if membro:
			self.listaDeMembros.append(Membro(0, membro, '', '', '[Sem Rotulo]', self.itemsDesdeOAno, self.itemsAteOAno, self.diretorioCache))
			self.listaDeRotulos.append("[Sem Rotulo]")
	...
	def carregarDadosCVLattes(self):
		...
		for membro in self.listaDeMembros:
			...
			indice += 1
			existe = membro.carregarDadosCVLattes()
			if existe == -1: #se nao existe
	...
	def obterParametro(self, parametro):
		if parametro == 'global-diretorio_de_saida' and self.diretorio:
